...
<html>
    <head>
        ...
        <script>
            ...
            function GrocerInfo(grocerid, password, name, age, email, phoneno, address) {
                document.getElementById('grocerid').value = grocerid;
            ...
            function toggleEdit() {
                ...
                const inputs = form.querySelectorAll("input:not(#grocerid):not([type=button]):not([type=submit])");
    ...
    <body>
        ...
        <div class="content-wrapper">
            ...
            <%
                Grocer grocer = (Grocer) request.getAttribute("grocer");
                MyUser user = (MyUser) request.getAttribute("user");
                if (grocer != null && user != null) {
            %>

            <form action="GrocerProfile?action=edit" method="POST" class="profile-form">
                <table>
                    <tr>
                        <td>Username: </td>
                        <td><input type="text" name="username" size="20" value="<%= grocer.getGrocerid()%>" readonly></td>
                    </tr>
                    <tr>
                        <td>Password: </td>
                        ...
                        <tr>
                            <td>Name: </td>
                            <td><input type="text" name="name" size="20" value="<%= grocer.getName()%>" <c:if test="${!isEditMode}">readonly</c:if>></td>
                        </tr>
                        <tr>
                            <td>Age: </td>
                            <td>
                                <button type="button" onclick="document.getElementById('ageInput').stepDown()" <c:if test="${!isEditMode}">disabled</c:if>>-</button>
                            <input type="number" name="age" id="ageInput" min="0" max="100" value="<%= grocer.getAge()%>" <c:if test="${!isEditMode}">readonly</c:if>>
                            <button type="button" onclick="document.getElementById('ageInput').stepUp()" <c:if test="${!isEditMode}">disabled</c:if>>+</button>
                            </td>
                        </tr>
                        <tr>
                            <td>Email: </td>
                            <td><input type="text" name="email" size="20" value="<%= grocer.getEmail()%>" <c:if test="${!isEditMode}">readonly</c:if>></td>
                        </tr>
                        <tr>
                            <td>Phone no: </td>
                            <td><input type="text" name="phoneno" size="20" value="<%= grocer.getPhoneno()%>" <c:if test="${!isEditMode}">readonly</c:if>></td>
                        </tr>
                        <tr>
                            <td>Location address: </td>
                            <td><input type="text" name="address" size="20" value="<%= grocer.getAddress()%>" <c:if test="${!isEditMode}">readonly</c:if>></td>
                        </tr>
                    </table>
