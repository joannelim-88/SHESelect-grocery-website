...
<html>
    <head>
        ...
        <title>Manage Grocer | SHESelect</title>
        <!--Web site tab icon --> 
        ...
        <script>
            ...
            function editGrocer(grocerid, name, age, email, phoneno, address) {
            ...
            function deleteGrocer(grocerid) {
                if (confirm("Are you sure you want to delete this grocer?")) {
                    
                    window.location.href = "DeleteGrocer?grocerid=" + grocerid;
                }
            }
    ...
    <body>
        ...
        <div class="content-wrapper">
            ...
            <br><br><br>
            <h1>Manage Grocer Information</h1>
            <%-- Show list of registered grocers --%>
            <div class="grocer-container">
                <%
                    List<Grocer> grocers = (List<Grocer>) request.getAttribute("grocerList");
                    if (grocers != null) {
                        for (Grocer grocer : grocers) {
                %>

                <div class="grocer-card">
                    <h3><%= grocer.getGrocerid()%></h3>
                    <div class="action-buttons">
                        <button onclick="editGrocer('<%= grocer.getGrocerid()%>', '<%= grocer.getName()%>', '<%= grocer.getAge()%>', '<%= grocer.getEmail()%>', '<%= grocer.getPhoneno()%>', '<%= grocer.getAddress()%>')">
                            <img src="images/edit icon.png" alt="Edit" class="icon">
                        </button>
                        <button onclick="deleteGrocer('<%= grocer.getGrocerid()%>')">
            ...
            <div id="editForm" class="dropdown-form">
                ...
                <form action="ManageGrocer" method="post">
                    <input type="hidden" id="grocerid" name="grocerid">
